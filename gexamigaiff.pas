...
type
  ...
  protected
    ...
    function HandleBody(ADecoder: TDecoder): Boolean;
  public
...
// Handle BODY tag
function TAmigaIffGraphic.HandleBody(ADecoder: TDecoder): Boolean;
type
  ...
  begin
    ...
    for i := 0 to FIffProperties.nPlanes-1 do begin
      ...
      if SwapEndian(Chunk^.ckID.tag) = IFF_ID_VDAT then begin
        ...
        ADecoder.Decode(vSource, vPlaneTarget, vSize, HeightPlaneSize);
        if ADecoder.DecoderStatus <> dsOk then
          ...
          repeat
            ...
            for y := 0 to Height-1 do begin
              ...
              if FIffProperties.CompressionType <> cmpRGB then begin
                case FIffProperties.CompressionType of
                  cmpByteRun1:
                    begin
                      ...
                      ADecoder.Decode(Pointer(FData.mdPos), Pointer(LineBuf),
                        NativeUInt(FData.mdEnd) - NativeUInt(FData.mdPos), AdjustedLineSize);
                      // Note that we can't test ADecoder.ADecoderStatus because it can return
                      // a status other than dsOk because we don't have the exact input size!
                      if ADecoder.DecompressedBytes <> Integer(AdjustedLineSize) then begin
                ...
                ADecoder.Decode(Pointer(FData.mdPos), Pointer(LineBuf),
                  NativeUInt(FData.mdEnd) - NativeUInt(FData.mdPos), AdjustedLineSize);
                if ADecoder.DecompressedBytes <> Integer(AdjustedLineSize) then
                  raise EgexInvalidGraphic.CreateFmt(gesDecompression, [IffType]);
          ...
          if FIffProperties.IffType = itPbm then begin
            ...
            for y := 0 to Height-1 do begin
              ...
              if FImageProperties.Compression = ctPackedBits then begin
                ...
                // Note that we can't test ADecoder.ADecoderStatus because it can return
                // a status other than dsOk because we don't have the exact input size!
                if ADecoder.DecompressedBytes <> Integer(PixelLineSize) then
                  raise EgexInvalidGraphic.CreateFmt(gesDecompression, [IffType]);
              end
